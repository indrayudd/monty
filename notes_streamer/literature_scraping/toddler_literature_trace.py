...
class ToddlerBehaviorLLMWrapper:
    ...
    def select_two_works(self, note: StoredObservation, query: str, candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
        note_tokens = set(re.findall(r"[a-z0-9']+", f"{note.name} {note.body} {query}".lower()))

        scored: list[tuple[float, dict[str, Any]]] = []
        for work in candidates:
            score = score_work_for_selection(work)
            title = str(work.get("display_name") or work.get("title") or "")
            abstract = extract_abstract_text(work)
            corpus = f"{title} {abstract}".lower()
            overlap = sum(1 for token in note_tokens if len(token) > 3 and token in corpus)
            score += overlap * 2.5
            scored.append((score, work))

        scored.sort(key=lambda item: item[0], reverse=True)
        return [work for _, work in scored[:2]]
...
def process_note(
    ...
    selected_candidates = analyzer.select_two_works(note, analysis.query, candidates)
    print(f"[trace] selected {len(selected_candidates)} work(s)", flush=True)
    lines: list[str] = []
    lines.append(_one_sentence(f"{note_name} query '{analysis.query}' selected for OpenAlex search."))
    for candidate in selected_candidates:
        work_id = (candidate.get("id") or "").rstrip("/").split("/")[-1]
        print(f"[trace] fetching work {work_id}", flush=True)
        full_work = openalex.get_work(work_id) if work_id else candidate
        lines.append(analyzer.summarize_work(full_work, note, analysis.query))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.note_file is None:
            ghost_db.initialize()
            print(
                f"[trace] polling first ingested note every {args.interval_seconds:.1f}s; "
        ...
        while True:
            if args.note_file is None:
                notes = ghost_db.read_notes(limit=1, order="asc")
                note = notes[0] if notes else None
            else:
                note = parse_note_file(args.note_file)
